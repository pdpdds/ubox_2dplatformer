...
uint8_t is_intersect(uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2)
{
    //entities[0].x + 6 < self->x + 10 && self->x + 6 < entities[0].x + 10 && self->y == entities[0].y;

    if ((x2 - 10 < x1) && (y2 - 13 < y1))
    {
        if ((x1 < x2 + 10) && (y1 - 13 < y2))
            return 1;
    }
...
{
    if ((entities[0].roomId == self->roomId) && !gameover_delay && lives && !invuln)
    {
        return is_intersect(entities[0].x, entities[0].y, self->x, self->y);
    }
